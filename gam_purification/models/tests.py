...
class TestEBM(unittest.TestCase):
    ...
    def test_ebm_simple(self):
        ...
        results_empirical = purify_ebm(ebm_global, use_density=False, dataset_name="toy",
                 move_name="uniform", X_train=X, X_means=np.mean(X, axis=0), X_stds=np.std(X, axis=0),
                 laplace=0, should_transpose=False)
        # Check Mains are centered.
        for pure_main in results_empirical['mains_moved'].values():
            assert np.abs(np.average(pure_main, weights=np.ones_like(pure_main))) < 1e-10
