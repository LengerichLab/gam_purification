...
def purify_xgb_uniform(xgb_mains, xgb_pairs, ebm_global, dataset_name,
                       X_means=None, X_stds=None):
    return purify_xgb(xgb_mains, xgb_pairs, ebm_global, False,
        dataset_name, "uniform", None, X_means, X_stds)


def purify_xgb_empirical(xgb_mains, xgb_pairs, ebm_global, dataset_name,
                         X_train, X_means=None, X_stds=None):
    return purify_xgb(xgb_mains, xgb_pairs, ebm_global, True,
        dataset_name, "empirical", X_train, X_means, X_stds, 0)


def purify_xgb_laplace(xgb_mains, xgb_pairs, ebm_global, dataset_name,
                       X_train, X_means=None, X_stds=None, laplace=1):
    return purify_xgb(xgb_mains, xgb_pairs, ebm_global, True,
        dataset_name, "laplace", X_train, X_means, X_stds, laplace)


def purify_xgb(xgb_mains, xgb_pairs, ebm_global, use_density,
               dataset_name, move_name, X_train=None,
               X_means=None, X_stds=None, laplace=0):
    ...
    return purify_all(xgb_mains, pairs, ebm_global, use_density,
        dataset_name, "xgb", move_name, X_train, X_means, X_stds, laplace)
...
def get_mains_and_pairs(xgb_pairs_raw, ebm_global, xgb_feature_names=None):
    ...
    if xgb_feature_names is None:
        xgb_feature_names = ["f{}".format(i) for i in range(len(ebm_global.feature_names))]

    feature_mapping = {
        xgb_feature_names[i]: ebm_global.feature_names[i] for i in range(len(ebm_global.feature_names))
    }
    for tree in xgb_pairs_raw:
        for entry in tree:
            ...
            if len(feats) < 2:
                ...
                marginal += 1
                min_val = np.max([float(list(x.values())[0][0]) for x in path])  # max of the mins
                max_val = np.min([float(list(x.values())[0][1]) for x in path])  # min of the maxes
                ...
                try:
                    xgb_mains[feature_mapping[feats[0]]][min_idx:max_idx] += float(leaf_val)
                except KeyError:
            ...
            if len(set(feats)) == 2:
                ...
                try:
                    if not swapped:
                        xgb_pairs[feat1, feat2][min_idx1:max_idx1, min_idx2:max_idx2] += float(leaf_val)
                    else:
                        xgb_pairs[feat2, feat1][min_idx2:max_idx2, min_idx1:max_idx1] += float(leaf_val)
                except KeyError:
                    if not swapped:
                        xgb_pairs[feat1, feat2] = np.zeros((len(good_vals1), len(good_vals2)))
                        xgb_pairs[feat1, feat2][min_idx1:max_idx1, min_idx2:max_idx2] += float(leaf_val)
                    else:
                        xgb_pairs[feat2, feat1] = np.zeros((len(good_vals2), len(good_vals1)))
                        xgb_pairs[feat2, feat1][min_idx2:max_idx2, min_idx1:max_idx1] += float(leaf_val)
